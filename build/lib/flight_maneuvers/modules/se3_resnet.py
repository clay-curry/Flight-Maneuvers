...
class SE3CNN(nn.Module):
    ...
    def __init__(self, pool: str = "snub_cube", res_features: str = '2_96', init: str = 'delta'):
        ...
        blocks = [
            R3PointConv(self.in_type, layer_types[0][0], bias=False, initialize=False)
        ]

        for i in range(len(layer_types) - 1):
            blocks.append(
                SE3_ResNetBlock(layer_types[i][0], layer_types[i][1], layer_types[i+1][0], 2, features=res_features)
            )
