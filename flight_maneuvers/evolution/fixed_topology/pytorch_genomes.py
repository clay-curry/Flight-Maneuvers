...
from flight_maneuvers.evolution.fixed_topology.layers.torch_layers import IncompatibleLayersError
from flight_maneuvers.evolution.fixed_topology.layers.torch_layers import PytorchLayer
...
class FixedTopologyGenome(BaseGenome):
    ...
    def visualize(self,
        ...
        This currently only works with genomes that use Pytorch layers.
        ...
        Attributes:
            ...
            **kwargs: Optional named arguments to be passed to
                :py:func:`Pytorch.keras.utils.plot_model`.
        ...
        for layer in self.layers:
            if not isinstance(layer, PytorchLayer):
        ...
        model = torch.nn.Sequential(layers=[layer.torch_layer for layer in self.layers])
        model(np.zeros(shape=self.input_shape))
        
        x = torch.zeros(shape=self.input_shape)
        y = self.layers(x)
